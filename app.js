...
function App() {
  ...
  async function loadRecipes() {
    try {
      ...
      if (!window.storage) {
        setDebugInfo('Storage not available - recipes will only be saved while this page is open');
      ...
      if (result && result.keys) {
        ...
        setDebugInfo('No recipes found - add your first one!');
      }
    } catch (err) {
      console.log('No existing recipes found:', err);
      setDebugInfo('Ready to add recipes!');
    } finally {
      setIsInitialLoad(false);
    }
  }
...
// Wait for React to be loaded, then render
